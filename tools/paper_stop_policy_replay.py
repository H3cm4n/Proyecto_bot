...
OUT = Path(os.getenv("PAPER_STOP_POLICY_OUT", "data/paper_stop_policy_replay.csv"))
SUMMARY_OUT = Path(os.getenv("PAPER_STOP_POLICY_SUMMARY_OUT", "data/paper_stop_policy_summary.csv"))
...
def is_bad_alignment(row: pd.Series) -> bool:
...
def simulate_trade(
    ...
    for _, row in rows.iterrows():
        ...
        # Hard stop: protección de catástrofe. Siempre activo.
        if pnl_pct <= hard_stop_pct:
            base.update(
                {
                    "sim_exit_reason": "HARD_STOP",
                    "sim_exit_bid": bid,
                    "sim_pnl_usd": pnl_usd,
                    "sim_pnl_pct": pnl_pct,
                    "sim_minutes_open": age_minutes,
        ...
        # Soft stop: pérdida moderada, pero no sale por ruido inicial.
        bad_alignment = is_bad_alignment(row)

        if (
            pnl_pct <= soft_stop_pct
            and age_minutes >= soft_stop_min_hold
            and (not soft_require_bad_alignment or bad_alignment)
        ):
            base.update(
                {
                    "sim_exit_reason": "SOFT_STOP",
                    "sim_exit_bid": bid,
                    "sim_pnl_usd": pnl_usd,
                    "sim_pnl_pct": pnl_pct,
                    "sim_minutes_open": age_minutes,
        ...
        # Trailing stop: solo protege después de que ya hubo ganancia.
        if (
        ...
        # Time exit: reduce exposición cuando el trade ya dio algo.
        if (
            time_exit_minutes > 0
...
def main() -> None:
    ...
    hard_stop_values = parse_float_csv("STOP_REPLAY_HARD_STOP_VALUES", "-15,-20,-25,-30")
    soft_stop_values = parse_float_csv("STOP_REPLAY_SOFT_STOP_VALUES", "-6,-8,-10")
    soft_min_hold_values = parse_int_csv("STOP_REPLAY_SOFT_MIN_HOLD_VALUES", "5,10,15")
    trailing_activate_values = parse_float_csv("STOP_REPLAY_TRAILING_ACTIVATE_VALUES", "5,10,15")
    trailing_floor_values = parse_float_csv("STOP_REPLAY_TRAILING_FLOOR_VALUES", "1,3,5")
    time_exit_minutes_values = parse_int_csv("STOP_REPLAY_TIME_EXIT_MINUTES_VALUES", "60,90,120")
    time_exit_min_pnl_values = parse_float_csv("STOP_REPLAY_TIME_EXIT_MIN_PNL_VALUES", "0,3,5")
    max_replay_minutes_values = parse_int_csv("STOP_REPLAY_MAX_MINUTES", "180")

    soft_require_bad_alignment = os.getenv("STOP_REPLAY_SOFT_REQUIRE_BAD_ALIGNMENT", "1") == "1"
    ...
    mask = (
        (results["hard_stop_pct"] == best["hard_stop_pct"])
        & (results["soft_stop_pct"] == best["soft_stop_pct"])
        & (results["soft_stop_min_hold"] == best["soft_stop_min_hold"])
        & (results["trailing_activate_pct"] == best["trailing_activate_pct"])
        & (results["trailing_floor_pct"] == best["trailing_floor_pct"])
        & (results["time_exit_minutes"] == best["time_exit_minutes"])
        & (results["time_exit_min_pnl_pct"] == best["time_exit_min_pnl_pct"])
    )

    best_details = results[mask].copy()

    print("\n=== DETALLE MEJOR POLÍTICA STOP ===")
    detail_cols = [
        "crypto_symbol",
        "outcome",
        "entry_price",
