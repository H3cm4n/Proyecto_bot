...
def simulate_forward(
    ...
    grouped = {
        ...
        for key, group in all_rows.groupby("signal_key")
    }
    ...
    for _, entry in entries.iterrows():
        ...
        entry_ask = entry.get("best_ask")

        if pd.isna(entry_ask) or entry_ask <= 0 or entry_ask >= 0.99:
            continue

        start = entry["observed_at_dt"]
...
def summarize(df: pd.DataFrame, group_col: str, min_count: int) -> pd.DataFrame:
    rows = []
...
def main() -> None:
    horizon_minutes = int(os.getenv("RESEARCH_REPLAY_HORIZON_MINUTES", "60"))
    take_profit = float(os.getenv("RESEARCH_TAKE_PROFIT", "0.02"))
    stop_loss = float(os.getenv("RESEARCH_STOP_LOSS", "0.02"))
    min_count = int(os.getenv("RESEARCH_REPLAY_MIN_COUNT", "3"))

    print("\n=== RESEARCH LANE REPLAY REPORT ===")
    print(f"Lane: BTC_CHEAP_CONVEX")
    ...
    for _, row in df.iterrows():
        ...
        item = row.to_dict()
        item["research_lane"] = classified["research_lane"]
        item["research_reason"] = classified["research_reason"]
        item["trade_direction"] = classified["trade_direction"]
        item["flow_support"] = flow_support_label(
            item.get("flow_bias"),
    ...
    if not research_rows:
        print("No hay candidatos BTC_CHEAP_CONVEX en candidate_journal.")
        return

    entries = pd.DataFrame(research_rows)
    entries["observed_at_dt"] = pd.to_datetime(entries["observed_at"], errors="coerce", utc=True)

    print(f"Candidatos research registrados: {len(entries)}")
    print(f"Señales únicas research: {entries['signal_key'].nunique()}")
    ...
    if results.empty:
        print("Todavía no hay suficientes puntos futuros para replay research.")
        return

    results["ask_bucket"] = results["best_ask"].apply(
        lambda x: bucket(x, [0.04, 0.06, 0.08, 0.10], ["<=0.04", "<=0.06", "<=0.08", "<=0.10", ">0.10"])
    )
    results["score_bucket"] = results["score"].apply(
        lambda x: bucket(x, [60, 65, 70, 80], ["<=60", "<=65", "<=70", "<=80", ">80"])
    )
    results["spread_bucket"] = results["spread"].apply(
        lambda x: bucket(x, [0.001, 0.003, 0.005], ["<=0.001", "<=0.003", "<=0.005", ">0.005"])
    )

    print(f"\nHipótesis research evaluadas: {len(results)}")
    ...
    for col, title in [
        ...
        ("spread_bucket", "POR SPREAD BUCKET"),
    ]:
